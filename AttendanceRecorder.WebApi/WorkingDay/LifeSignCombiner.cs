...
public class LifeSignCombiner(LifeSignConfig config)
{
    ...
    public IEnumerable<IntervalDto> CombineLifeSigns(IOrderedEnumerable<TimeOnly> lifeSigns)
    {
        var activeIntervals = lifeSigns.Aggregate(
        ...
        var intervals = activeIntervals.Aggregate(
            new Stack<IntervalDto>(),
            (stack, interval) =>
            {
                if (stack.Count == 0)
